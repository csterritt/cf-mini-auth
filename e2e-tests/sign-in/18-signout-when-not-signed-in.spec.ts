...
import { verifyOnStartupPage, verifyOnSignInPage } from '../support/page-verifiers'

// Test: sign out when not signed in (no session cookie)
test('sign out endpoint when not signed in (no session cookie)', async ({ page }) => {
...
test('sign out endpoint with invalid session cookie', async ({ page, context }) => {
